...
import org.emu.camunda.config.KafkaProperties;
...
class CamundaServerKafkaResourceIT {
    ...
    void setup() {
        ...
        CamundaServerKafkaResource kafkaResource = new CamundaServerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/camunda-server-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/camunda-server-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
